...
class freq_model:
    def __init__(self, out_lib_name: str="freq_model_lib.so", compile: bool=True):
        """
    ...
    def reset(self, u: np.ndarray, L: np.ndarray, t_step: np.ndarray):
        """
        Reset the simulation back to initial state.
        """
        self.current_time = self.start_time
        self.next_time = self.current_time + self.step_time
        self.freq_model_system.initialize(u, L, t_step,  self.x_est, self.x)
    ...
    def one_step(self, u: np.ndarray, L: np.ndarray, t_step: np.ndarray):
        """
        Run one step of simulation
        """
        self.freq_model_system.one_step(u, L, t_step,  self.x_est, self.x)
